...
export const registerAccount = async (req, res) => {
  ...
  if (!username || !password || !role_id) {
    return res
      ...
      .json({
        message:
          "Vui lòng nhập đầy đủ thông tin, bao gồm username, password và role!",
      });
  }
...
export const login = async (req, res) => {
  ...
  try {
    ...
    const roleResult = await db.query(
      "SELECT role_id FROM account_roles WHERE acc_id = $1",
      [user.acc_id]
    );
    if (roleResult.rows.length === 0) {
      return res
        .status(500)
        .json({ message: "Không tìm thấy vai trò của người dùng!" });
    }
    ...
    const token = jwt.sign(
      { userId: user.acc_id, username: user.username, role_id },
      JWT_SECRET,
      { expiresIn: "1h" }
    );
